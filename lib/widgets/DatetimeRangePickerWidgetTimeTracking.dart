...
class DatetimeRangePickerWidgetTimeTracking extends StatefulWidget {
  @override
  _DatetimeRangePickerWidgetTimeTrackingState createState() => _DatetimeRangePickerWidgetTimeTrackingState();
}

class _DatetimeRangePickerWidgetTimeTrackingState extends State<DatetimeRangePickerWidgetTimeTracking> {
    ...
    if (dateTimeFrom == null) {
      ...
      if (dateTimeUntil != null) {
        ...
        if (dateTimeUntil.difference(dateTimeFrom).inDays >= 1) {
          regisTT = true;
          fromT = DateFormat('MM/dd/yyyy HH:mm').format(dateTimeFrom);
          return DateFormat('MM/dd/yyyy HH:mm').format(dateTimeFrom);
        }
        if (dateTimeUntil.difference(dateTimeFrom).inDays <= -1) {
          regisTT = false;
  ...
  String getTextUntil() {
    if (dateTimeUntil == null) {
      ...
      if (dateTimeFrom != null) {
        ...
        if (dateTimeUntil.difference(dateTimeFrom).inDays >= 1) {
          regisTT = true;
          untilT = DateFormat('MM/dd/yyyy HH:mm').format(dateTimeUntil);
          return DateFormat('MM/dd/yyyy HH:mm').format(dateTimeUntil);
        }
        if (dateTimeUntil.difference(dateTimeFrom).inDays <= -1) {
          regisTT = false;
