...
class DatetimeRangePickerWidgetModifyContract extends StatefulWidget {
  @override
  _DatetimeRangePickerWidgetModifyContractState createState() => _DatetimeRangePickerWidgetModifyContractState();
}

class _DatetimeRangePickerWidgetModifyContractState extends State<DatetimeRangePickerWidgetModifyContract> {
  DateTime dateTimeUntil = DateFormat('MM/dd/yyyy HH:mm').parse(untilMCMod);
  DateTime dateTimeFrom = DateFormat('MM/dd/yyyy HH:mm').parse(fromMCMod);
  String getTextFrom() {
    if (dateTimeFrom == null) {
      ...
      if (dateTimeUntil.difference(dateTimeFrom).inDays >= 1) {
        editCts = true;
        fromMCMod = DateFormat('MM/dd/yyyy HH:mm').format(dateTimeFrom);
        return DateFormat('MM/dd/yyyy HH:mm').format(dateTimeFrom);
      }
      if (dateTimeUntil.difference(dateTimeFrom).inDays <= -1) {
        editCts = false;
  ...
  String getTextUntil() {
    if (dateTimeUntil == null) {
      ...
      if (dateTimeUntil.difference(dateTimeFrom).inDays >= 1) {
        editCts = true;
        untilMCMod = DateFormat('MM/dd/yyyy HH:mm').format(dateTimeUntil);
        return DateFormat('MM/dd/yyyy HH:mm').format(dateTimeUntil);
      }
      if (dateTimeUntil.difference(dateTimeFrom).inDays <= -1) {
        editCts = false;
  ...
  Widget build(BuildContext context) => HeaderWidget(
        ...
        child: Row(
          children: [
            Expanded(
              child: ButtonWidget(
                text: (dateTimeFrom == null) ? fromMCMod : getTextFrom(),
            ...
            Expanded(
              child: ButtonWidget(
                text: (dateTimeUntil == null) ? untilMCMod : getTextUntil(),
